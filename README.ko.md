...
많은 일상 사진에서 진짜 마음을 움직이는 것은 ‘큰 장면’이 아니라 하나의 자세, 짧은 교감, 말로 설명하기 어려운 작은 유머입니다. 전체를 채색하면 이런 디테일이 평범해지고, 장식이 너무 많으면 무심코 적어 둔 듯한 사적인 느낌이 가려집니다.

**Panel 067**은 빨강과 파랑, 두 가지 손그림 잉크만 고정해서 사용하며 주요 동작과 관계를 한 페이지의 사적인 관찰 기록으로 압축합니다. 비뚤어진 선과 구어체 짧은 문장, 옅은 종이의 넓은 여백이 따뜻함과 유머를 함께 지키고 사진을 범용 일러스트처럼 포장하지 않습니다.

### 이런 상황에 적합합니다

- **평범한 생활 속 재미있는 순간을 기록하고 싶을 때:** 사람과 동물, 물건 사이의 동작과 관계가 이야기의 중심이 되어야 합니다.
- **아주 적은 색으로도 뚜렷한 정체성을 원할 때:** 고정된 빨강·파랑 두 잉크가 통일된 인상을 만들며 사진을 여러 색의 팔레트로 확장하지 않습니다.
- **글이 광고가 아니라 관찰처럼 보여야 할 때:** 작은 손글씨 단어, 구어체 또는 의성어가 윤곽과 여백에 자연스럽게 호응합니다.
...
> XXD Panel 067으로 이 이미지를 작업하고, 먼저 가장 어울리는 구성과 크기를 추천해 줘.
...
## 빠른 판단: XXD Panel 067이 나에게 맞을까요?

| 궁금한 점 | 이 스타일이 주는 답 |
|---|---|
| **얻게 되는 결과** | 고정된 빨간색과 파란색 잉크를 사용하여 유머러스하고 온화한 일상의 관찰을 기록하세요 |
| **한눈에 알아볼 특징** | 빨간색과 파란색 고정 · 손으로 그린 이중 잉크 · 사적인 관찰 · 어린애 같은 유머 · 넓은 여백 |
...
고정된 빨간색과 파란색 드로잉 잉크만을 사용하여 재미 있고 감동적이거나 은유적인 일상의 순간을 하나의 서사 중심과 몇 가지 단서로 압축합니다. 순진한 대사와 대화의 단편, 그리고 넓은 백지밭은 따뜻하고 유머러스한 사적인 관찰을 만들어낸다.

```text
일상적인 서술 중심 식별 → 전체 장면 및 장식 정보 제거 → 빨강-파랑 잉크로 윤곽, 동작 및 관계 단순화 → 몇 가지 기호 및 방향 표시로 이야기 지원 → 넓은 연한 종이 공간을 통해 분위기 설정 → 대화, 의성어 또는 느린 단편 추가
```

## 완성 스타일을 알아보는 특징

- 빨간색과 파란색은 두 개의 고정된 기본 잉크입니다. 사진은 그것들을 다색 팔레트로 확장하지 않습니다.
- 선은 순진한 비례 과장이지만 정확한 관계를 통해 가늘고, 구부러지고, 끊어지거나 다시 그려집니다.
- 오직 하나의 서사 중심과 몇 개의 의미 있는 기호, 행동 흔적, 방향선만 남습니다.
- 주제는 필드를 채우지 않습니다. 거의 흰색에 가까운 종이 바탕에는 공기, 유머, 부드러움이 담겨 있습니다.
- 손으로 쓴 작은 단어, 대화 문구, 소리 또는 여운 한 문장이 에코 윤곽선, 화살표 및 빈 공간을 나타냅니다.

미적 제약과 금지 항목은 [중국어 원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 런타임 어댑터는 이번 전달 변수만 처리합니다. [Skill 워크플로](SKILL.md) · [영문 런타임 어댑터](references/xxd-panel-067-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 067의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-067 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-067 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-067 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-067 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-067 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 067 적용, 공통 설정은 한 번만 확정
/xxd-panel-067 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-067`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-067-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-067-prompt.zh-CN.md)
...
| **[xxd-panel-067](https://github.com/nevertoday/xxd-panel-067)** | 검은 주도형 · 거대한 여백 · 망점 소멸 · 선적 사유 · 생각의 파편 |
...
| **병사급** | `xxd-panel-NNN` (현재 001–067) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
