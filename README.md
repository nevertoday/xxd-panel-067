...
**Panel 067** 适合这类任务：需要保留原图核心内容，同时希望最终视觉达到以下方向：**用固定红蓝双墨记录幽默、温柔的日常观察**。它先从用途与期望结果出发，再决定构图、尺寸、文字和具体输出形式。
...
只用固定红、蓝两种手绘墨色，把照片中有趣、动人或带隐喻的日常瞬间概括成一个主要叙事中心和少量线索。童真线条、口语短句与大片浅纸留白共同形成温柔幽默的私人观察。

```text
识别日常中的主要叙事中心 → 删除完整场景与装饰信息 → 用红蓝双墨概括轮廓、动作和关系 → 让少量符号与方向线参与叙事 → 以大片浅纸留白建立情绪 → 加入口语、拟声或余味短句
```

## 成品中最容易识别的风格特点

- 红色与蓝色是固定的两种主墨色，不从照片扩展成多色色盘。
- 线条纤细、歪斜、可断开或重复描画，比例略有天真夸张但关系准确。
- 只保留一个主要叙事中心和极少量有意义的符号、动作痕迹或方向线。
- 主体不填满画面，大面积近白纸底承担空气、幽默与温柔情绪。
- 小型手写短词、口语、拟声或一句余味文案与轮廓、箭头和留白互相呼应。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-067-prompt.en.md)
...
| **[xxd-panel-067](https://github.com/nevertoday/xxd-panel-067)** | 黑色主形 · 极大留白 · 网点消散 · 禅意思考 · 哲思碎片 |
...
| **士兵级** | `xxd-panel-NNN`（当前 001–067） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
