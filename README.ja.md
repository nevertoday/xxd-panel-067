...
# 🦁 XXD Panel 067｜日常ユーモア画

### 赤と青の固定インクを使用して、ユーモラスで優しい日常の観察を記録します
...
日常写真の多くで本当に心を動かすのは、「大きな場面」ではなく、一つの姿勢、短いやり取り、うまく言葉にできない少しのユーモアです。すべてに色を塗るとそうした細部が平凡になり、装飾を増やしすぎると、ふと書き留めたような私的な感覚が隠れてしまいます。

**Panel 067** は手描きの赤と青の 2 色だけを使い、中心となる動きと関係を一ページの私的な観察メモへ圧縮します。傾いた線、話し言葉の短文、淡い紙の大きな余白が温かさとユーモアを一緒に残し、写真をありきたりなイラストに仕立てません。

### このような場面に適しています

- **何気ない暮らしの面白い瞬間を残したい：** 人、動物、物の間にある動きと関係を物語の中心にしたい。
- **ごく少ない色でも明確な個性が欲しい：** 赤と青の固定した 2 色のインクで統一感を作り、写真から多色のパレットへ広げません。
- **文字を広告ではなく観察のように見せたい：** 小さな手書きの単語、話し言葉、擬音が輪郭や余白と自然に呼応します。
...
> XXD Panel 067 でこの画像を処理して、まず最適な構図とサイズを提案して。
...
**キーワード:** 固定の赤と青 · 手描きの二重インク · 個人的な観察 · 子どもらしいユーモア · 広い領域の空白

Panel 067 は、赤と青の 2 つの手描きインクのみを使用して、非常に薄い紙に手作業によるミスのある斜めの断続的な線で核となるシーンを再現し、注意深くデザインされた個人的な観察ノートのように見せています。

<!-- xxd-panel-benefit:start -->
## すぐに判断：XXD Panel 067 はあなたに合う？

| 知りたいこと | このスタイルの答え |
|---|---|
| **得られる完成作品** | 赤と青の固定インクを使用して、ユーモラスで優しい日常の観察を記録します |
| **ひと目で分かる特徴** | 固定の赤と青 · 手描きの二重インク · 個人的な観察 · 子どもらしいユーモア · 広い領域の空白 |
...
固定された赤と青の描画インクのみを使用して、面白い、感動的な、または比喩的な日常の瞬間を 1 つの物語の中心といくつかのヒントに凝縮します。素朴なセリフ、会話の断片、そして広い淡い紙のフィールドが、温かくユーモラスな個人的な観察を生み出します。

```text
日常の物語の中心を特定する → 完全なシーンと装飾的な情報を削除する → 赤と青のインクで輪郭、アクション、関係を単純化する → いくつかの記号と方向記号でストーリーをサポートする → 淡い紙の広い空間を通して雰囲気を確立する → 会話、擬音語、または余韻の断片を追加する
```

## 完成作品を見分けるポイント

- 赤と青は 2 つの固定プライマリ インクです。写真はそれらを多色のパレットに拡張しません。
- 線は細い、曲がっている、壊れている、または描き直されており、素朴な比例を強調しながらも正確な関係を持っています。
- 唯一の物語の中心と、いくつかの意味のあるシンボル、行動の痕跡、または方向線だけが残ります。
- 件名がフィールドを埋めることはありません。ほぼ白に近い紙の地面には、空気、ユーモア、そして優しさが伝わってきます。
- 手書きの小さな単語、会話のフレーズ、音、または余韻に残る一文が、輪郭、矢印、空白をエコーします。

美的制約と避けるべき表現は[中国語の原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは今回の納品条件だけを扱います。 [Skill ワークフロー](SKILL.md) · [英語ランタイムアダプター](references/xxd-panel-067-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 067 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-067 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-067 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-067 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-067 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 067 を適用し、共通設定は一度だけ解決
/xxd-panel-067 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-067`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-067-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-067-prompt.zh-CN.md)
...
| **[xxd-panel-067](https://github.com/nevertoday/xxd-panel-067)** | 黒い主形 · 巨大な余白 · 網点の消散 · 禅的思考 · 思考の断片 |
...
| **兵士級** | `xxd-panel-NNN`（現在 001–067） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
